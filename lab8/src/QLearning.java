...
public class QLearning {
    ...
    private final int mazeHeight = 7;
    ...
    public void init() {
        File file = new File("dataMaze.txt");
    ...
    void findPath(int starea) {

        int nxtState = getPolicyFromState(starea);
        System.out.println("From state " + starea + " goto state " + nxtState);
        for (int i = 0; i < mazeHeight; i++) {
            for (int j = 0; j < mazeWidth; j++) {
                if (mazeWidth * i + j == starea)
                    System.out.print("P ");
                else
        ...
        if (!isFinalState(starea))
            findPath(getPolicyFromState(starea));
    }
}
